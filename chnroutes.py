...
def generate_routeros(metric):
    ...
    rfile=open('routes.txt','w')
    for ip,_,mask in results:
        route_item="/ip firewall address-list add list=chnroutes address= %s/%s\n"%(ip,mask)
        rfile.write(route_item)
    rfile.close()
    print "Usage: execute the script of the newly created routes.txt " \
          "create lists of IP addresses grouped together under chnroutes"
...
if __name__=='__main__':
    ...
    parser.add_argument('-p','--platform',
                        ...
                        "win, android, routeros. openvpn by default.")
